...
public class ServerMessageManager {
	...
	// AbstractMessage sendMessage = (AbstractMessage)
	// updateExistingQuestion.updateExistingQuestion(newQuestionTooAdd);
	// return sendMessage;
	// }
	//
	// /**
	// * @author gal
	// * @param msg type of QuestionMessage which contain the string "Questions" ans
	// the subject of the questions as payload
	// * @return {@link AbstractMessage} of QuestionMessage filled with question
	...
	// getQuestions.questions(questionMessage.getThisQuestionsSubject().getSubjectID());
	//
	// if (questions.size() ==0) return message.getMessage("error-Qeustions",new
	// Exception("No Questions in this subject")); // return Exception
	// else if (questions.size() >= 1) return
	// message.getOkGetMessage("ok-get-questions".split("-"),questions); // found
	// questions for this subject, return them
	// return message.getMessage("error-Qesutions",new Exception("Error in finding
	// Qesutions"));
	// }
	//
	// /**
	// * @author gal
	// * @param msg type of UserSubjectMessage wich contain string "UserSubjects"
	// and User payload
	...
	// getUserSubjects.subjects(userSubjects.getUser().getUserID());
	// userSubjects.setSubjects(subjects);
	// if (subjects.size() ==0) return message.getMessage("error-userSubjects",new
	// Exception("No subjects for the User")); // this user has now teaching
	// subject, return Exception
	// else if (subjects.size() >= 1) return
	// message.getOkGetMessage("ok-get-usersubjects".split("-"),userSubjects); //
	// this user has teaching subjects, return his HIS subjects
	// return message.getMessage("error-userSubjects",new Exception("Error in
	// finding userSubjects"));
	// }
	//
	//
	// /*
	...
	// */
	// private static AbstractMessage handleFetUserMessage(String[] msgContent,
	// AbstractMessage msg) {
	// switch(msgContent[2]) {
	// case "name":
	...
	// usersMap.put(user.getUserID(),user.getUserFirstName()+"
	...
	// * @return An exam message containing an arraylist of statistics assembled by
	// teacherID
	// */
	...
	// ArrayList<Statistic> examList =
	// getExams.getExamStatsByTeacherID(idMessage.getId());//TODO:Make a query in
	// getManager to handle getting all the exams assembled by a single teacher
	// ExamMessage message=(ExamMessage)
	// factory.getMessage("ok-get-solvedExamByTeacherId", examList);
	// return message;
	// }
	//
	// /**
	// *
	// * @param msg type of LoginMessage which contain string, and loginInfo
	// payload.
	...
	// Question newQuestionTooAdd = recievedNewQuestion.getQuestions().get(0); //
	// ArrayList<Question> will contain only 1 new question
	// SetInDB putQuestion = new SetInDB();
	// AbstractMessage sendMessage = (AbstractMessage)
	// putQuestion.AddNewQuestion(newQuestionTooAdd);
	// return null;
	// }
	//
	...
	// }
	//
	// private static AbstractMessage handleDeleteQuestionMessage(AbstractMessage
	// msg) {
	...
	// AbstractMessage sendMessage = (AbstractMessage)
	// deletesQuestion.deleteTheQuestion(deleteQuestion);
	// return sendMessage;
	// }
	//
	...
	// AbstractMessage sendMessage = (AbstractMessage)
	// deletesExam.deleteTheExam(deleteExam);
	// return sendMessage;
	// }
	//
	// private static AbstractMessage handlePutQuestionInExamMessage(AbstractMessage
	// msg) {
	...
	// SetInDB putExam = new SetInDB();
	// AbstractMessage sendMessage = (AbstractMessage) putExam.addQuestionToExam(id,
	// examQuestions);
	// return sendMessage;
	// }
	//
	...
	/**
	 * @author gal this method called when updating an existing question in DB
	 * @param msg
	 * @return
	 */
	private static AbstractMessage handleSetQuestionMessage(AbstractMessage msg) {
	...
	private static AbstractMessage handleQuestionsMessage(AbstractMessage msg) {
		...
		ArrayList<Question> questions = getQuestions
				.questions(questionMessage.getThisQuestionsSubject().getSubjectID());
	 ...
	 * @author gal
	 * @param msg
	 *            type of UserSubjectMessage wich contain string "UserSubjects" and
	 *            User payload
	 * @return AbstrackMessage of userSubjects filled with speficic user subjects.
	 */
	private static AbstractMessage handleUserSubjectsMessage(AbstractMessage msg) {
	 ...
	 * @author Alon Ben-yosef
	 * @param msg
	 *            of UserIDMessage type expected
	 * @return An exam message containing an arraylist of exams assembled by
	 *         teacherID
	 */
	private static AbstractMessage handleGetExamByTeacherID(AbstractMessage msg) {
		// TODO:Convert getFromDB to singleton
		...
		MessageFactory factory = MessageFactory.getInstance();
		GetFromDB getExams = new GetFromDB();
		ArrayList<Exam> examList = getExams.exams();// TODO:Make a query in getManager to handle getting all the exams
													// assembled by a single teacher
